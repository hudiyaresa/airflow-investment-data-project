...
class ExtractTransform:
    def _categories(incremental, date):
        if incremental:
            data = _extract(connection_id = 'staging_db', table_name='staging.categories', incremental = True, date = date)
            object_name = f'categories-{date}.csv'

        else:
            data = _extract(connection_id = 'staging_db', table_name='staging.categories', incremental = False)
    ...
    def _customers(incremental, date):
        if incremental: 
            data = _extract(connection_id = 'staging_db', table_name = 'staging.customers', incremental = True, date = date)
            object_name = f'customers-{date}.csv'   

        else:
            data = _extract(connection_id = 'staging_db', table_name='staging.customers', incremental = False)
    ...
    def _products(incremental, date):
        if incremental:
            data = _extract(connection_id = 'staging_db', table_name='staging.products', incremental = True, date = date)
            object_name = f'products-{date}.csv'
        else:
            data = _extract(connection_id = 'staging_db', table_name='staging.products', incremental = False)
    ...
    def _inventory(incremental, date):
        if incremental:
            data = _extract(connection_id = 'staging_db', table_name='staging.inventory', incremental = True, date = date)
            object_name = f'inventory-{date}.csv'
        else:
            data = _extract(connection_id = 'staging_db', table_name='staging.inventory', incremental = False)
    ...
    def _orders(incremental, date):
        if incremental:
            data = _extract(connection_id = 'staging_db', table_name='staging.orders', incremental = True, date = date)
            object_name = f'orders-{date}.csv'
        else:
            data = _extract(connection_id = 'staging_db', table_name='staging.orders', incremental = False)
    ...
    def _orderlines(incremental, date):
        if incremental:
            data = _extract(connection_id = 'staging_db', table_name='staging.orderlines', incremental = True, date = date)
            object_name = f'orderlines-{date}.csv'
        else:
            data = _extract(connection_id = 'staging_db', table_name='staging.orderlines', incremental = False)
    ...
    def _cust_hist(incremental, date):
        if incremental:
            data = _extract(connection_id = 'staging_db', table_name='staging.cust_hist', incremental = True, date = date)
            object_name = f'cust_hist-{date}.csv'   
        else:
            data = _extract(connection_id = 'staging_db', table_name='staging.cust_hist', incremental = False)
    ...
    def _order_status_analytic(incremental, date):
        if incremental:
            data = _extract(connection_id = 'staging_db', table_name='staging.order_status_analytic', incremental = True, date = date)
            object_name = f'order_status_analytic-{date}.csv'
        else:
            data = _extract(connection_id = 'staging_db', table_name='staging.order_status_analytic', incremental = False)
    ...
    def _customers_history(incremental, date):
        if incremental:
            data = _extract(connection_id = 'staging_db', table_name='staging.customer_orders_history', incremental = True, date = date)
            object_name = f'customers_history-{date}.csv'
        else:
            data = _extract(connection_id = 'staging_db', table_name='staging.customer_orders_history', incremental = False)

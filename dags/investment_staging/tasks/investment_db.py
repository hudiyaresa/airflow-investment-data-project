...
@task_group
def investment_db(incremental):
    """
    Task group to handle the extraction and loading of Investment database data.
    ...
    def extract():
        """
        Task group to handle the extraction of data from Investment database.
        """
        # Get the list of tables to extract from Airflow Variable
        table_to_extract = eval(Variable.get('list_investment_table'))
        ...
        for table_name in table_to_extract:
            SparkSubmitOperator(
                task_id=table_name,
                conn_id="spark-conn",
                application="dags/investment_staging/tasks/components/extract.py",
                application_args=[
                    f'{table_name}',
                    f'{incremental}',
                    f'{DATE}'
    ...
    def load():
        """
        Task group to handle the loading of data into Investment database.
        """
        # Get the list of tables to load and their primary keys from Airflow Variable
        table_to_load = eval(Variable.get('list_investment_table'))
        table_pkey = eval(Variable.get('pkey_investment_table'))
        ...
        for table_name in table_to_load:
            current_task = SparkSubmitOperator(
                task_id=table_name,
                conn_id="spark-conn",
                application="dags/investment_staging/tasks/components/load.py",
                application_args=[
                    table_name,
